...
export default function CancelOfferPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/orderbook/offer/cancel", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        toast.success(`Offer cancelled successfully! Order ID: ${data.orderId}`);
        // Reset form
        setFormData({
          orderId: "",
          signature: ""
        ...
        toast.error(`Failed to cancel offer: ${error.error}`);
      }
    } catch (error) {
      toast.error("An error occurred while cancelling the offer");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <div className="flex items-center justify-between space-y-2">
        <h2 className="text-3xl font-bold tracking-tight">Cancel Offer</h2>
      </div>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" disabled={loading} className="w-full" variant="destructive">
              {loading ? "Cancelling Offer..." : "Cancel Offer"}
            </Button>
          </form>
        </CardContent>
      </Card>
